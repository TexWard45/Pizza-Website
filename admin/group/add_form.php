...
<div class="modal-dialog">
    <form class="add-form" onsubmit="return false;">
        ...
        <div class="modal-header">
            <h5 class="modal-title"> Thêm nhóm tài khoản </h5>
...
<script> 
    $(".add-form").submit(function() {
        if (confirm("Xác nhận thêm nhóm tài khoản?")) {
            $.ajax({
                url: "/admin/group/add.php", 
                method: "POST",
                data: {display: $(".display")[0].value},
                success: function(response){
                    if (response == "success") {
                        alert("Thêm nhóm tài khoản thành công!");
                        $(".modal").modal("hide");
                        reloadAdmin();
                    }else if (response == "conflict") {
                        alert("Tên nhóm tài khoản đã tồn tại!");
                    }
                }
            });
        }
