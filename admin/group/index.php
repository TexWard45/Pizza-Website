...
if (isset($_POST['display'])) {
    $groups = Group::getBySearch($_POST['display']);
}else {
    $groups = Group::getAll();
}
?>
...
<div class="p-3">
    <div class="d-flex justify-content-between">
        <div class="title">
            <span class="h3"> Quản lý nhóm tài khoản </span>
        </div>
    ...
    <?php
        if (count($groups) == 0) :
    ?>
        <div> Không tìm thấy nhóm tài khoản nào! </div>
    <?php else: ?>
    ...
    <table class="table">
        ...
        ?>
            <tr>
                <td>
                    <input class="form-check-input checkbox" type="checkbox" data-id="<?=$group->getId()?>">
                </td>
                <td> <?=$group->getId()?> </td>
...
<script>
    $(".item").click(function() {
        ...
        let arr = getSelects();
        $.ajax({
            url: "/admin/group/" + operation + "_form.php", 
            method: "POST",
            data: {id_array: arr},
