...
<div class="modal-dialog">
    <form class="remove-form" onsubmit="return false;">
        ...
        <div class="modal-header">
            <h5 class="modal-title"> Xác nhận xóa đế bánh pizza? </h5>
...
<script> 
    $(".remove-form").submit(function() {
        if (confirm("Bạn có chắc muốn xóa?")) {
            $.ajax({
                url: "/admin/base/remove.php", 
                method: "POST",
                data: {id_array: <?=json_encode($idArray)?>},
                success: function(response){
                    if (response == "success") {
                        alert("Xóa đế bánh pizza thành công!");
                        $(".modal").modal("hide");
                        reloadAdmin();
                    }else if (response == 'exists') {
                        alert("Không thể xóa đế bánh pizza đang được sử dụng bởi bánh pizza khác!");
                        $(".modal").modal("hide");
                    }
                }
            });
