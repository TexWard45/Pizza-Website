...
if (isset($_POST['display'])) {
    $bases = Base::getBySearch($_POST['display']);
}else {
    $bases = Base::getAll();
}
?>
...
<div class="p-3">
    <div class="d-flex justify-content-between">
        <div class="title">
            <span class="h3"> Quản lý đế bánh pizza </span>
        </div>
    ...
    <?php
        if (count($bases) == 0) :
    ?>
        <div> Không tìm thấy đế bánh nào! </div>
    <?php else: ?>
    ...
    <table class="table">
        ...
        ?>
            <tr>
                <td>
                    <input class="form-check-input checkbox" type="checkbox" data-id="<?=$base->getId()?>">
                </td>
                <td> <?=$base->getId()?> </td>
                <td> <?=$base->getDisplay()?> </td>
            </tr>
        <?php endforeach; ?>
    </table>
    </div>
...
<script>
    $(".item").click(function() {
        ...
        let arr = getSelects();
        $.ajax({
            url: "/admin/base/" + operation + "_form.php", 
            method: "POST",
            data: {id_array: arr},
