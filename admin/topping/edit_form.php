...
<div class="modal-dialog">
    <form class="edit-form" onsubmit="return false;">
        ...
        <div class="modal-header">
            <h5 class="modal-title"> Sửa nhân bánh pizza </h5>
...
<script> 
    $(".edit-form").submit(function() {
        if (confirm("Xác nhận sửa nhân bánh pizza?")) {
            $.ajax({
                url: "/admin/topping/edit.php", 
                method: "POST",
                data: {id: "<?=$id?>", display: $(".display")[0].value},
                success: function(response){
                    if (response == "success") {
                        alert("Sửa nhân bánh pizza thành công!");
                        $(".modal").modal("hide");
                        reloadAdmin();
                    }
                }
