...
if (isset($_POST['display'])) {
    $toppings = Topping::getBySearch($_POST['display']);
}else {
...
<div class="p-3">
    <div class="d-flex justify-content-between">
        <div class="title">
            <span class="h3"> Quản lý nhân bánh pizza </span>
        </div>
    ...
    <?php
        if (count($toppings) == 0) :
    ?>
        <div> Không tìm thấy nhân bánh nào! </div>
    <?php else: ?>
    ...
    <table class="table">
        ...
        ?>
            <tr>
                <td>
                    <input class="form-check-input checkbox" type="checkbox" data-id="<?=$topping->getId()?>">
                </td>
                <td> <?=$topping->getId()?> </td>
                <td> <?=$topping->getDisplay()?> </td>
            </tr>
        <?php endforeach; ?>
    </table>
    <div>
...
<script>
    $(".item").click(function() {
        ...
        let arr = getSelects();
        $.ajax({
            url: "/admin/topping/" + operation + "_form.php", 
            method: "POST",
            data: {id_array: arr},
