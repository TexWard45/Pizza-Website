...
<div class="modal-dialog">
    <form class="add-form" onsubmit="return false;">
        ...
        <div class="modal-header">
            <h5 class="modal-title"> Thêm nhân bánh pizza </h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
                <div class="fw-bold"> Tên nhân bánh: </div>
...
<script> 
    $(".add-form").submit(function() {
        if (confirm("Xác nhận thêm nhân bánh pizza?")) {
            $.ajax({
                url: "/admin/topping/add.php", 
                method: "POST",
                data: {display: $(".display")[0].value},
                success: function(response){
                    if (response == "success") {
                        alert("Thêm nhân bánh pizza thành công!");
                        $(".modal").modal("hide");
                        reloadAdmin();
                    }else if (response == "conflict") {
                        alert("Tên nhân bánh pizza đã tồn tại!");
                    }
                }
            });
        }
