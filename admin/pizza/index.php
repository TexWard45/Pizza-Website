...
if (isset($_POST['display'])) {
    $pizzas = Pizza::getBySearch($_POST['display']);
}else {
    $pizzas = Pizza::getAll();
}
...
<div class="p-3">
    <div class="d-flex justify-content-between">
        <div class="title">
            <span class="h3"> Quản lý bánh pizza </span>
        </div>
    ...
    <?php
        if (count($pizzas) == 0) :
    ?>
        <div> Không tìm thấy bánh pizza nào! </div>
    <?php else: ?>
    ...
    <table class="table">
        ...
        ?>
            <tr>
                <td>
                    <input class="form-check-input checkbox" type="checkbox" data-id="<?=$pizza->getId()?>">
                </td>
                <td> <?=$pizza->getId()?> </td>
                <td> <img src="/img/pizza/<?=$pizza->getImage()?>" width="100px" height="100px"> </td>
                <td> <?=$pizza->getDisplay()?> </td>
                <td> <?=Category::getById($pizza->getCategoryId())->getDisplay()?> </td>
...
<script>
    $(".item").click(function() {
        ...
        let arr = getSelects();
        $.ajax({
            url: "/admin/pizza/" + operation + "_form.php", 
            method: "POST",
            data: {id_array: arr},
