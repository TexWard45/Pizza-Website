...
<div class="modal-dialog">
    <form class="add-form" onsubmit="return false;" method="post" enctype="multipart/form-data" action="/game/pizza/add.php">
        <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title"> Thêm bánh pizza </h5>
        ...
        <div class="modal-body">
                ...
                <select class="form-select group" name="category">
                    ...
                    ?>
                        <option value="<?=$category->getId()?>"> <?=$category->getDisplay()?> </option>
                    <?php endforeach; ?>
                </select>
                ...
                <?php
                    ...
                    <div class="form-check">
                        <input class="form-check-input topping" type="checkbox" name="topping-<?=$topping->getId();?>" id="topping-<?=$topping->getId();?>" data-id="<?=$topping->getId();?>">
                ...
                ?>
                    <div class="form-check">
                        <input class="form-check-input size" type="checkbox" name="size-<?=$size->getId();?>" id="size-<?=$size->getId();?>" data-id="<?=$size->getId();?>">
                        ...
                        <div class="base-div" style="display: none"> 
                            ...
                            ?>
                                <div class="form-check">
                                    <input class="form-check-input base base-<?=$size->getId()?>" type="checkbox" name="base-<?=$base->getId();?>" id="base-<?=$base->getId();?>" data-id="<?=$base->getId();?>">
                                    ...
                                    <div class="detail-div" style="display: none"> 
                                        <div class="d-flex mb-2">
                                            ...
                                            <input class="form-control price-<?=$size->getId()?>-<?=$base->getId()?>" type="text" id="price-<?=$base->getId()?>">
                                        </div>

                                        <div class="d-flex">
                                            <input type="text" readonly="" class="form-control-plaintext" value="Số lượng">
                                            <input class="form-control amount-<?=$size->getId()?>-<?=$base->getId()?>" type="number" id="amount-<?=$base->getId()?>">
                                        </div>
                                    </div>
                                </div>
                            <?php endforeach;?>
...
<script> 
    $(".add-form").submit(function() {
        ...
        if (confirm("Xác nhận thêm bánh pizza?")) {
