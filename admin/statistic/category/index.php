...
if (isset($_POST['startdate']) || isset($_POST['enddate'])) {
    if (!empty($_POST['startdate']) && !empty($_POST['enddate'])) {
        ...
        $sql = "SELECT `order_detail`.order_id, category.id as category_id, order_detail.price, order_detail.quantity
        FROM `order_detail` 
...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        $obj = array($row['order_id'], $row['category_id'], $row['price'], $row['quantity']);
...
<div class="p-3">
    <div class="d-flex justify-content-between">
        <div class="title">
            <span class="h3"> Thống kê doanh thu theo danh mục bánh pizza <?=$searchdate?> </span>
        </div>
            ...
            foreach ($categories as $category) {
                ...
                foreach ($orderDetails as $orderDetail) {
                    ...
                    if ($statusDetail->getStatusId() == 6) {
                        $totalRevenue1 += $price * $quantity;
                        $totalQuantity1 += $quantity;
                    }

                    if ($statusDetail->getStatusId() < 6) {
                        $totalRevenue2 += $price * $quantity;
