...
<div class="modal-dialog">
    <form class="add-form" onsubmit="return false;">
        ...
        <div class="modal-header">
            <h5 class="modal-title"> Thêm danh mục bánh pizza </h5>
...
<script> 
    $(".add-form").submit(function() {
        if (confirm("Xác nhận thêm danh mục bánh pizza?")) {
            $.ajax({
                url: "/admin/category/add.php", 
                method: "POST",
                data: {display: $(".display")[0].value},
                success: function(response){
                    if (response == "success") {
                        alert("Thêm danh mục bánh pizza thành công!");
                        $(".modal").modal("hide");
                        reloadAdmin();
                    }else if (response == "conflict") {
                        alert("Tên danh mục bánh pizza đã tồn tại!");
                    }
                }
            });
        }
